...
abstract contract Ownable is Context {
  ...
  */
  ...
  /**
   * @dev calculates the transfer amount by deductiong the tax fees.
   */
  function calculateTransferAmount(uint256 _amount) private view returns (uint256) {
        return _amount.sub(calculateTaxFee(_amount));
  }
  /**
   * @dev deactivates the transfer fees on all transfers
   */
  function removeFees() public onlyOwner {
        _noFees = true;
    }
    /**
   * @dev activates the transfer fees on all transfers
   */
  function addFees() public onlyOwner {
        _noFees = false;
    }
     /**
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal {
        require(sender != address(0), "Transfers from the zero address are not possible");
        require(recipient != address(0), "Transfers to the zero address are not possible");
        require(amount > 0, "Transfer amount must be greater than zero");
        _balances[sender] = _balances[sender].sub(amount, "Transfer amount exceeds balance");
       if (_noFees || (_isExcluded[sender] && _isExcluded[recipient]) || _isExcludedFromAllFees[sender]) {
